...
func main() {
	...
	if installState.Mode == "" {
		...
		log.Println("Install mode: local (default, first run)")
	} else {
	...
	// System tray
	state.appTray = tray.New(app, func() {
	...
	transcriptionWorker, err := speechkit.NewTranscriptionWorker(speechkit.TranscriptionWorkerConfig{
		...
		Runner: speechkit.NewTranscriptionRunner(
			...
			speechkitStoreAdapter{store: feedbackStore, state: state},
		),
		Output:   desktopTranscriptOutput{handler: clipHandler, interceptor: quickActions},
		Observer: state,
	})
	if err != nil {
