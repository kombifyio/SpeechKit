...
func TestDesktopQuickNoteServiceOpenCaptureReplacesExistingEmptyPlaceholderNote(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "test.db")
	feedbackStore, err := store.New(store.StoreConfig{
		Backend:           "sqlite",
		SQLitePath:        dbPath,
	...
	firstID, err := service.OpenCapture(context.Background())
	if err != nil {
		t.Fatalf("OpenCapture() first error = %v", err)
	}
	secondID, err := service.OpenCapture(context.Background())
	if err != nil {
		t.Fatalf("OpenCapture() second error = %v", err)
	}
	if firstID == secondID {
		t.Fatalf("OpenCapture() reused placeholder note %d, want a new capture note", firstID)
	}
	if got, want := host.closeCaptureCall, 1; got != want {
		t.Fatalf("host.closeCaptureCall = %d, want %d when reopening capture", got, want)
	}
	...
	if count != 1 {
		t.Fatalf("QuickNoteCount = %d, want 1 after reopening capture", count)
	}
}

func TestDesktopQuickNoteServiceOpenEditorClosesActiveCaptureAndRemovesPlaceholder(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "test.db")
	feedbackStore, err := store.New(store.StoreConfig{
		Backend:           "sqlite",
		SQLitePath:        dbPath,
	...
	if _, err := service.OpenCapture(context.Background()); err != nil {
		t.Fatalf("OpenCapture() error = %v", err)
	}

	if err := service.OpenEditor(55); err != nil {
		t.Fatalf("OpenEditor() error = %v", err)
	}
	if got, want := host.closeCaptureCall, 1; got != want {
		t.Fatalf("host.closeCaptureCall = %d, want %d before opening editor", got, want)
	}
	if got, want := host.editorNoteID, int64(55); got != want {
		t.Fatalf("host.editorNoteID = %d, want %d", got, want)
	}
	...
	runtime := state.runtimeStateLocked()
	if runtime.quickNoteMode {
		t.Fatal("runtime.quickNoteMode = true, want false after opening editor")
	}
	if runtime.quickCaptureMode {
		t.Fatal("runtime.quickCaptureMode = true, want false after opening editor")
	}
	...
	if count != 0 {
		t.Fatalf("QuickNoteCount = %d, want 0 after opening editor from empty capture", count)
	}
}
