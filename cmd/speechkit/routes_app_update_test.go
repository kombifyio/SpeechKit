...
func TestAppUpdateRoutesDownloadInstallerAndOpenIt(t *testing.T) {
	...
	updateMu.Lock()
	updateVersion = "0.19.0"
	updateURL = "https://example.com/releases/tag/v0.19.0"
	updateDownloadURL = server.URL + "/SpeechKit-Setup-v0.19.0.exe"
	updateDownloadName = "SpeechKit-Setup-v0.19.0.exe"
	updateDownloadSize = int64(len(payload))
	updateChecked = testNow()
	updateMu.Unlock()
	t.Cleanup(resetCachedLatestReleaseForTest)

	form := url.Values{"version": {"0.19.0"}}
