...
// ValidateLocalProviderActivation checks that the local whisper.cpp
// model path is valid, the binary is present, and the model file is
// available. Returns a specific error describing what is missing so
// the caller can present an actionable hint.
