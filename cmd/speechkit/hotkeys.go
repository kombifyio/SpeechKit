...
// Submit injects a synthetic hotkey event into the manager's event stream.
// Used by the wake-word dispatcher to convert a "Hey Quby" detection into
// the same KeyDown event the user would have generated by pressing the
// configured mode hotkey. Drops the event if the buffered channel is full
// (matches the per-mode forwardLoop behaviour — the input controller
// already tolerates dropped events from a real hotkey under contention).
func (m *modeHotkeyManager) Submit(event hotkey.Event) {
	select {
	case m.events <- event:
	default:
	}
