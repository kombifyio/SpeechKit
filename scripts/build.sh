...
GO_LDFLAGS="-X ${GO_MODULE_PATH}/internal/config.managedHFDefaultOptIn=${MANAGED_HF_DEFAULT} -X ${GO_MODULE_PATH}/internal/config.managedDopplerDefaultProject=${MANAGED_DOPPLER_PROJECT} -X ${GO_MODULE_PATH}/internal/config.managedDopplerDefaultConfig=${MANAGED_DOPPLER_CONFIG}"
...
go build -ldflags "$GO_LDFLAGS" -o "$BUNDLE_EXE" ./cmd/voicetool/
