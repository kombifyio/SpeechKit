...
export interface AgentAudioVisualizerRadialProps {
  ...
  /**
   * Manual normalized audio level used when no audio track is available.
   */
  level?: number;
  /**
...
export function AgentAudioVisualizerRadial({
  ...
  const syntheticBands = useMemo(
    () => buildSyntheticBands(_barCount, level),
    [_barCount, level],
  );
  const bands = useMemo(
    () => (audioTrack ? volumeBands : syntheticBands),
    [audioTrack, syntheticBands, volumeBands],
  );
