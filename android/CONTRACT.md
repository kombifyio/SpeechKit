...
| `keyboard/voicepanel` + HeliBoard fork integration | planned | Inline voice-input panel behind a small `VoiceInputHost` seam (fork lives in its own GPL repo, consumed as submodule) |
...
| `net/ConnectionProfileSource.kt` | present | Flavor-bound default `ConnectionProfile` resolution (B-M2b): `oss` = always system tier; `kombify` = stored server profile, else system tier |
| Pairing/auth provisioning | planned | Server URL + token pairing (`speechkit.pairing.v1`); hosted access via the Companion co-install claim flow. The former `cloud/` Zitadel module was an empty stub and has been removed — the platform is Auth0, and the solo app deliberately holds no account tokens. |
