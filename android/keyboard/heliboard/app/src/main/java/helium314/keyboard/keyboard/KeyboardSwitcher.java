...
public final class KeyboardSwitcher implements KeyboardState.SwitchActions {
    ...
    private View mSpeechKitFeaturesView;
    private View mSpeechKitAssistPanel;
    private boolean mSpeechKitFeaturesShowing = false;
    ...
    private void setMainKeyboardFrame(
        ...
        if (mSpeechKitFeaturesView != null) {
            mSpeechKitFeaturesView.setVisibility(View.GONE);
            mSpeechKitFeaturesShowing = false;
        if (mSpeechKitAssistPanel != null) mSpeechKitAssistPanel.setVisibility(View.GONE);
        }
    }

    public void toggleSpeechKitFeatures() {
        if (mSpeechKitFeaturesView == null) return;

        if (mSpeechKitFeaturesShowing) {
            // Hide feature panel, show keyboard
            mSpeechKitFeaturesView.setVisibility(View.GONE);
            mKeyboardView.setVisibility(View.VISIBLE);
            mSpeechKitFeaturesShowing = false;
        if (mSpeechKitAssistPanel != null) mSpeechKitAssistPanel.setVisibility(View.GONE);
        } else {
            // Show feature panel, hide keyboard
            mKeyboardView.setVisibility(View.GONE);
            mEmojiPalettesView.setVisibility(View.GONE);
            mClipboardHistoryView.setVisibility(View.GONE);
            mSpeechKitFeaturesView.setVisibility(View.VISIBLE);
            mSpeechKitFeaturesShowing = true;

            // Wire up feature panel buttons
            new helium314.keyboard.speechkit.SpeechKitFeaturesController(mLatinIME).attach(mSpeechKitFeaturesView);
        }
    }

    public void showAssistPanel(String question, String answer) {
        if (mSpeechKitAssistPanel == null) return;

        // Hide keyboard, show assist panel
        mKeyboardView.setVisibility(View.GONE);
        mEmojiPalettesView.setVisibility(View.GONE);
        mClipboardHistoryView.setVisibility(View.GONE);
        if (mSpeechKitFeaturesView != null) mSpeechKitFeaturesView.setVisibility(View.GONE);

        // Populate content
        android.widget.TextView questionView = mSpeechKitAssistPanel.findViewById(R.id.speechkit_assist_question);
        android.widget.TextView answerView = mSpeechKitAssistPanel.findViewById(R.id.speechkit_assist_answer);
        if (questionView != null) questionView.setText(question);
        if (answerView != null) answerView.setText(answer);

        // Wire buttons
        android.widget.Button insertBtn = mSpeechKitAssistPanel.findViewById(R.id.speechkit_assist_insert);
        android.widget.Button copyBtn = mSpeechKitAssistPanel.findViewById(R.id.speechkit_assist_copy);
        android.widget.Button closeBtn = mSpeechKitAssistPanel.findViewById(R.id.speechkit_assist_close);

        if (insertBtn != null) insertBtn.setOnClickListener(v -> {
            mLatinIME.getCurrentInputConnection().commitText(answer + " ", 1);
            hideAssistPanel();
        });
        if (copyBtn != null) copyBtn.setOnClickListener(v -> {
            android.content.ClipboardManager clipboard = (android.content.ClipboardManager)
                mLatinIME.getSystemService(android.content.Context.CLIPBOARD_SERVICE);
            clipboard.setPrimaryClip(android.content.ClipData.newPlainText("SpeechKit", answer));
            android.widget.Toast.makeText(mLatinIME, "Kopiert", android.widget.Toast.LENGTH_SHORT).show();
            hideAssistPanel();
        });
        if (closeBtn != null) closeBtn.setOnClickListener(v -> hideAssistPanel());

        mSpeechKitAssistPanel.setVisibility(View.VISIBLE);
    }

    public void hideAssistPanel() {
        if (mSpeechKitAssistPanel != null) {
            mSpeechKitAssistPanel.setVisibility(View.GONE);
        }
        mKeyboardView.setVisibility(View.VISIBLE);
    }
    ...
    public View onCreateInputView(@NonNull Context displayContext, final boolean isHardwareAcceleratedDrawingEnabled) {
        ...
        mSpeechKitFeaturesView = mCurrentInputView.findViewById(R.id.speechkit_features_view);
        mSpeechKitAssistPanel = mCurrentInputView.findViewById(R.id.speechkit_assist_panel);
