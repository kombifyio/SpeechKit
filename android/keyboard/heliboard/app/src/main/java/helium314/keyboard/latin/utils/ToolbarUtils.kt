...
fun getCodeForToolbarKey(key: ToolbarKey) = Settings.getInstance().getCustomToolbarKeyCode(key) ?: when (key) {
    ...
    // SpeechKit custom actions
    SPEECHKIT_MENU -> KeyCode.SPEECHKIT_FEATURES
    DICTATE -> KeyCode.SPEECHKIT_DICTATE
    ASSIST -> KeyCode.SPEECHKIT_ASSIST
}
...
enum class ToolbarKey {
    ...
    // SpeechKit custom toolbar keys
    SPEECHKIT_MENU, DICTATE, ASSIST
}
...
val defaultToolbarPref by lazy {
    val default = listOf(SPEECHKIT_MENU, DICTATE, ASSIST, SETTINGS, VOICE, EMOJI, CLIPBOARD, UNDO, REDO, SELECT_WORD, COPY, PASTE, LEFT, RIGHT)
...
val defaultPinnedToolbarPref by lazy {
    // SpeechKit: pin our core buttons so they stay visible even during suggestions
    val pinned = setOf(SPEECHKIT_MENU, VOICE)
    entries.filterNot { it == CLOSE_HISTORY }.joinToString(Separators.ENTRY) {
        it.name + Separators.KV + (it in pinned)
    }
}
