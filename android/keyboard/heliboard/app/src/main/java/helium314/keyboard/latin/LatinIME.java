...
public class LatinIME extends InputMethodService implements
    ...
    private helium314.keyboard.speechkit.SpeechKitVoiceInput mSpeechKitVoice;
    ...
    public void onEvent(@NonNull final Event event) {
        ...
        if (keyCode == KeyCode.VOICE_INPUT || keyCode == KeyCode.SPEECHKIT_DICTATE) {
            if (mSpeechKitVoice == null) {
                mSpeechKitVoice = new helium314.keyboard.speechkit.SpeechKitVoiceInput(this);
            }
            mSpeechKitVoice.toggle();
            return;
        }
        if (keyCode == KeyCode.SPEECHKIT_ASSIST) {
            onSpeechKitAssist();
            return;
        }
        if (keyCode == KeyCode.SPEECHKIT_FEATURES) {
            mKeyboardSwitcher.toggleSpeechKitFeatures();
            return;
        }
    ...
    public void onSpeechKitDictate() {
        if (mSpeechKitVoice == null) {
            mSpeechKitVoice = new helium314.keyboard.speechkit.SpeechKitVoiceInput(this);
        }
        mSpeechKitVoice.toggle();
    ...
    public void onSpeechKitAssist() {
        // Assist mode: record -> transcribe -> send to LLM -> show answer in keyboard
        if (mSpeechKitVoice == null) {
            mSpeechKitVoice = new helium314.keyboard.speechkit.SpeechKitVoiceInput(this);
        }
        mSpeechKitVoice.toggleAssist();
    }

    public void showAssistResult(String question, String answer) {
        mKeyboardSwitcher.showAssistPanel(question, answer);
    }
