 ...
 *
 * The model is **not shipped**: release artifacts never carry model weights,
 * and this module has no `assets/` directory, so the constructor throws until
 * something downloads the file into `filesDir/models/`. That makes this the
 * opt-in detector, not the default one — [LevelVadDetector] is what the system
 * assistant and the keyboard endpoint with, because it works on a fresh
 * install.
 */
