...
func TestPublicSDKDoesNotImportInternalPackages(t *testing.T) {
	...
	allowlist := map[string]map[string]string{
		...
		// Adapter over the reference app's config loader so Load applies the
		// exact same defaults/backfills (one loader, no reimplementation);
		// exported signatures use the public hostconfig.Config only.
		"hostconfig": {
			internalPrefix + "config":            "reference config loader + normalisers",
			internalPrefix + "voiceagentprofile": "built-in Voice Agent profile IDs",
		},
	}
