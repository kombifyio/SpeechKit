...
func (m *mockProvider) Kind() ProviderKind             { return m.kind }
func (m *mockProvider) Health(_ context.Context) error { return m.err }

// Router parity (sdkparity.RunTTSRouterParity) is covered in tts_test.go.
...
func TestRouterLocalOnlyAllowsLocalProviderKind(t *testing.T) {
	local := &mockProvider{name: "openedai-kokoro", kind: ProviderKindLocalProvider}
	cloud := &mockProvider{name: "huggingface", kind: ProviderKindCloudProvider}
