...
func TestAssemblyAI_SyncIntegrationTranscribe(t *testing.T) {
	...
	provider := New(key, "")
	resolved := stt.ResolveTranscribeOptions("assemblyai", "stt.assemblyai.universal", stt.TranscribeOpts{Language: "en"}, nil, nil)
	if !provider.syncEligible(wav, stt.TranscribeOpts{}, resolved) {
	...
	defer cancel()
	start := time.Now()
	result, err := provider.transcribeSync(ctx, wav, start, stt.TranscribeOpts{
		Language: "en",
	...
	}, stt.ResolveTranscribeOptions("assemblyai", "stt.assemblyai.universal", stt.TranscribeOpts{Language: "en"}, nil, nil))
	if err != nil {
