...
func TestAssemblyAI_SyncTranscribe_SendsPromptContextAndKeyterms(t *testing.T) {
	...
	result, err := p.Transcribe(context.Background(), syncTestClip(t, 2*time.Second), stt.TranscribeOpts{
		Language: "de",
...
func TestAssemblyAI_SyncFailureFallsBackToAsync(t *testing.T) {
	...
	result, err := p.Transcribe(context.Background(), syncTestClip(t, 2*time.Second), stt.TranscribeOpts{Language: "de"})
	if err != nil {
		t.Fatalf("Transcribe: %v", err)
	}
	if syncCalls != 1 {
...
func TestAssemblyAI_SyncSkippedForAsyncOnlyFeatures(t *testing.T) {
	...
	p := newSyncTestProvider(syncServer.URL, asyncServer.URL)
	_, err := p.Transcribe(context.Background(), syncTestClip(t, 2*time.Second), stt.TranscribeOpts{
		Language: "de",
...
func TestAssemblyAI_SyncSkippedWhenDisabledOrTooLong(t *testing.T) {
	...
	resolved := stt.ResolvedTranscribeOptions{Language: "de"}
	if p.syncEligible(syncTestClip(t, 2*time.Second), stt.TranscribeOpts{}, resolved) != true {
		t.Fatal("short clip should be sync-eligible")
	}
	if p.syncEligible(syncTestClip(t, 2*time.Second), stt.TranscribeOpts{}, stt.ResolvedTranscribeOptions{}) {
		t.Fatal("auto/unset language must keep the async flow (sync has no language_detection)")
	}
	if p.syncEligible(syncTestClip(t, 130*time.Second), stt.TranscribeOpts{}, resolved) {
		t.Fatal("clips over 120s must not be sync-eligible")
	}
	if p.syncEligible(syncTestClip(t, 30*time.Millisecond), stt.TranscribeOpts{}, resolved) {
		t.Fatal("clips under 80ms must not be sync-eligible")
	}
	p.DisableSync = true
	if p.syncEligible(syncTestClip(t, 2*time.Second), stt.TranscribeOpts{}, resolved) {
		t.Fatal("DisableSync must force the async flow")
	}
	p.DisableSync = false
	if p.syncEligible(syncTestClip(t, 2*time.Second), stt.TranscribeOpts{Model: "universal-2"}, resolved) {
