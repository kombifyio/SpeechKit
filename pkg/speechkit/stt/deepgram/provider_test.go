...
func TestDeepgram_Transcribe_Diarization(t *testing.T) {
	...
	result, err := p.Transcribe(context.Background(), []byte("pcm"), stt.TranscribeOpts{
		Language: "de",
...
func TestDeepgram_Transcribe_PopulatesWordConfidence(t *testing.T) {
	...
	p := newTestDeepgramProvider(server.URL)
	result, err := p.Transcribe(context.Background(), []byte("pcm"), stt.TranscribeOpts{Language: "de"})
	if err != nil {
		t.Fatalf("Transcribe: %v", err)
	}
...
func TestDeepgram_Transcribe_DetectLanguageIsIgnoredForCodeSwitching(t *testing.T) {
	...
	_, err := p.Transcribe(context.Background(), []byte("pcm"), stt.TranscribeOpts{Language: "de"})
	if err != nil {
		t.Fatalf("Transcribe: %v", err)
	}
...
func TestDeepgram_Transcribe_UsesLegacyKeywordsForNova2(t *testing.T) {
	...
	p.Validation = testValidation
	p.client.Timeout = 5 * time.Second
	_, err := p.Transcribe(context.Background(), []byte("pcm"), stt.TranscribeOpts{Keyterms: []string{"AcmeOS"}})
	if err != nil {
		t.Fatalf("Transcribe: %v", err)
	}
