...
func (p *Provider) Transcribe(ctx context.Context, audio []byte, opts stt.TranscribeOpts) (*stt.Result, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint, bytes.NewReader(stt.EnsureTranscriptionWAV(audio)))
	if err != nil {
	...
	lang := stt.FirstNonEmptyTrimmed(parsed.detectedLanguage(), apiLanguage, "multi")
...
func (p *Provider) deepgramListenEndpoint(model string, resolved stt.ResolvedTranscribeOptions) (string, error) {
	endpoint, err := netsec.BuildEndpoint(stt.FirstNonEmptyTrimmed(p.BaseURL, deepgramBaseURL), "v1/listen", p.Validation)
	if err != nil {
		return "", fmt.Errorf("deepgram endpoint: %w", err)
	}
	q := url.Values{}
	...
	if resolved.Speaker.WantsDiarization() {
		...
		diarizationModel := stt.FirstNonEmptyTrimmed(resolved.Speaker.DiarizationModel, p.DiarizationModel, "latest")
...
func (p *Provider) resolveOptions(model string, opts stt.TranscribeOpts) stt.ResolvedTranscribeOptions {
	...
	return stt.ResolveTranscribeOptions("deepgram", deepgramProfileID(model), opts, providerDefaults, providerOverrides)
}
...
func deepgramResolvedLanguage(resolved stt.ResolvedTranscribeOptions) string {
...
func deepgramLanguageSource(resolved stt.ResolvedTranscribeOptions) string {
...
// words are present.
func (r deepgramResponse) transcriptWords() []stt.WordConfidence {
	for _, channel := range r.Results.Channels {
		if len(channel.Alternatives) == 0 {
			continue
		}
		...
		out := make([]stt.WordConfidence, 0, len(words))
		for _, word := range words {
			out = append(out, stt.WordConfidence{
				Text:       stt.FirstNonEmptyTrimmed(word.PunctuatedWord, word.Word),
