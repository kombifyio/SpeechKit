...
func (p *Provider) StartFluxTurnStream(ctx context.Context, opts FluxStreamOptions, format speaker.AudioFormat) (*FluxTurnStream, error) {
	...
	model := stt.FirstNonEmptyTrimmed(opts.Model, DeepgramFluxModelMulti)
...
func (p *Provider) deepgramFluxEndpoint(model string, opts FluxStreamOptions, format speaker.AudioFormat) (string, error) {
	// Flux lives on /v2/listen; the v1 path rejects these models outright.
	endpoint, err := netsec.BuildEndpoint(stt.FirstNonEmptyTrimmed(p.BaseURL, deepgramBaseURL), "v2/listen", p.Validation)
	if err != nil {
...
func (e deepgramFluxEvent) err() error {
	...
	detail := stt.FirstNonEmptyTrimmed(e.Description, e.Message, "unknown error")
