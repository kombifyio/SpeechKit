...
func (p *Provider) StartDictationStream(ctx context.Context, opts speechkit.DictationStreamOptions, format speaker.AudioFormat) (speechkit.DictationStream, error) {
	...
	language := stt.FirstNonEmptyTrimmed(opts.Language, p.LanguageOverride, deepgramCodeSwitchingLanguage())
...
func (p *Provider) deepgramStreamingEndpoint(model, language string, format speaker.AudioFormat) (string, error) {
	...
	if language := normalizedDeepgramLanguage(stt.FirstNonEmptyTrimmed(language, p.LanguageOverride)); language != "" {
...
func (p *Provider) deepgramDictationStreamingEndpoint(model, language string, opts speechkit.DictationStreamOptions, format speaker.AudioFormat) (string, error) {
	endpoint, err := netsec.BuildEndpoint(stt.FirstNonEmptyTrimmed(p.BaseURL, deepgramBaseURL), "v1/listen", p.Validation)
	if err != nil {
...
func (r deepgramStreamingResponse) dictationEvent(provider, model, fallbackLanguage string, sessionID uint64, sequence, _ int64) speechkit.DictationStreamEvent {
	...
	event := speechkit.DictationStreamEvent{
		...
		IsFinal:        isFinal,
		Language:       stt.FirstNonEmptyTrimmed(r.Channel.DetectedLanguage, fallbackLanguage, deepgramCodeSwitchingLanguage()),
		Provider:       provider,
		Model:          model,
	}
	if len(r.Channel.Alternatives) == 0 {
