...
func (p *GoogleSTTProvider) StartSpeakerStream(ctx context.Context, opts speaker.Options, format speaker.AudioFormat) (speaker.SpeakerStream, error) {
	...
	if projectID == "" {
		projectID = firstNonEmptyTrimmed(os.Getenv("GOOGLE_CLOUD_PROJECT"), resolveSecret("GOOGLE_CLOUD_PROJECT"))
	}
	if projectID == "" {
		if projectIDErr != nil {
...
func (p *GoogleSTTProvider) resolveGoogleStreamingCredentials() (*auth.Credentials, error) {
	credentialsJSONEnv := firstNonEmptyTrimmed(p.STTCredentialsJSONEnv, envGoogleSTTCredentialsJSON)
	applicationCredentialsEnv := firstNonEmptyTrimmed(p.ApplicationCredentialsEnv, envGoogleApplicationCreds)

	if raw := firstNonEmptyTrimmed(os.Getenv(credentialsJSONEnv), resolveSecret(credentialsJSONEnv)); raw != "" {
...
func googleStreamingCredentialsPresent(applicationCredentialsEnv, credentialsJSONEnv string) bool {
	applicationCredentialsEnv = firstNonEmptyTrimmed(applicationCredentialsEnv, envGoogleApplicationCreds)
	credentialsJSONEnv = firstNonEmptyTrimmed(credentialsJSONEnv, envGoogleSTTCredentialsJSON)
	...
	if strings.TrimSpace(resolveSecret(applicationCredentialsEnv)) != "" {
		return true
	}
	return strings.TrimSpace(resolveSecret(credentialsJSONEnv)) != ""
}
