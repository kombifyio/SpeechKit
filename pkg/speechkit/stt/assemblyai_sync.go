...
func (p *AssemblyAIProvider) syncEligible(wav []byte, opts TranscribeOpts, resolved ResolvedTranscribeOptions) bool {
	if p.DisableSync {
		return false
	}
...
func (p *AssemblyAIProvider) syncModelHeader() string {
	return firstNonEmptyTrimmed(p.SyncModel, assemblyAIFlagshipModel)
}

// transcribeSync performs the single-request sync transcription.
func (p *AssemblyAIProvider) transcribeSync(ctx context.Context, wav []byte, start time.Time, opts TranscribeOpts, resolved ResolvedTranscribeOptions) (*Result, error) {
	endpoint, err := netsec.BuildEndpoint(firstNonEmptyTrimmed(p.SyncBaseURL, assemblyAISyncBaseURL), "transcribe", p.Validation)
	if err != nil {
	...
	result := &Result{
		...
		Language:   firstNonEmptyTrimmed(cfg.LanguageCode, resolved.Language, "de"),
	...
	if len(response.Words) > 0 {
		result.Words = make([]WordConfidence, 0, len(response.Words))
		for _, word := range response.Words {
			result.Words = append(result.Words, WordConfidence{
				Text:       word.Text,
...
func (p *AssemblyAIProvider) Warm(ctx context.Context) error {
	...
	endpoint, err := netsec.BuildEndpoint(firstNonEmptyTrimmed(p.SyncBaseURL, assemblyAISyncBaseURL), "warm", p.Validation)
	if err != nil {
