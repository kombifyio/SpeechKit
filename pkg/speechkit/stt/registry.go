...
//     provider name, endpoint, and constructor — the mapping that was
//     previously duplicated across host call sites.
//   - BuildRouter assembles a Router from a set of enabled providers so the
//     Device- and Server-Targets share one assembly path while each keeps its
//     own config-resolution specifics (mirrors pkg/speechkit/tts.BuildRouter).
package stt
...
// (DeepgramOptions is the existing Listen-option type; the assembly struct is
...
func BuildRouter(cfg RouterConfig, enabled EnabledProviders) (router *Router, ok bool, notes []string) {
	...
	if o := enabled.Groq; o != nil {
		...
		cloud = append(cloud, NewOpenAICompatibleProvider("groq", "https://api.groq.com/openai", o.APIKey, model))
	...
	if o := enabled.OpenAI; o != nil {
		...
		cloud = append(cloud, NewOpenAICompatibleProvider("openai", "https://api.openai.com", o.APIKey, model))
		notes = append(notes, "STT: OpenAI registered (model="+model+")")
	}
	if o := enabled.Deepgram; o != nil {
		p := NewDeepgramProvider(o.APIKey, o.Model)
	...
	if o := enabled.AssemblyAI; o != nil {
		p := NewAssemblyAIProvider(o.APIKey, o.Models)
	...
	if o := enabled.Google; o != nil {
		p := NewGoogleSTTProvider(o.APIKey, o.Model)
	...
	var local STTProvider
	if o := enabled.Local; o != nil {
		local = NewLocalProvider(o.Port, o.ModelPath, o.GPU)
	...
	if profileID := strings.TrimSpace(cfg.PreferredProfileID); profileID != "" {
		if preferred := providerNameFromProfileID(profileID); preferred != "" {
			ordered := prioritizeProviderProfile(cloud, profileID)
