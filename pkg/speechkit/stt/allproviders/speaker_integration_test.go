...
func TestIntegrationDeepgramDiarization(t *testing.T) {
	key := testutil.RequireEnvOrSkip(t, "DEEPGRAM_API_KEY", "Deepgram speaker diarization integration requires live Deepgram credentials.")
...
func TestIntegrationAssemblyAIDiarization(t *testing.T) {
	key := testutil.RequireEnvOrSkip(t, "ASSEMBLYAI_API_KEY", "AssemblyAI speaker diarization integration requires live AssemblyAI credentials.")
	audio := loadSpeakerIntegrationAudio(t)
...
// Google is an opt-in BYOK provider: Kombify CI holds no Google credentials,
// so the Google gates skip unless a user explicitly requires live config.
func TestIntegrationGoogleSTTDiarization(t *testing.T) {
	key := ""
	for _, envName := range []string{google.EnvAPIKey, google.EnvCloudAPIKey, google.EnvLegacyAPIKey} {
		if key = strings.TrimSpace(os.Getenv(envName)); key != "" {
			break
		}
	}
	if key == "" {
		testutil.SkipOrFailExplicitMissingConfig(t, google.EnvAPIKey, "Google STT diarization integration requires your own Google STT credentials.")
	}
	audio := loadSpeakerIntegrationAudio(t)
...
func TestIntegrationDeepgramSpeakerStreaming(t *testing.T) {
	key := testutil.RequireEnvOrSkip(t, "DEEPGRAM_API_KEY", "Deepgram speaker streaming integration requires live Deepgram credentials.")
...
func TestIntegrationAssemblyAISpeakerStreaming(t *testing.T) {
	key := testutil.RequireEnvOrSkip(t, "ASSEMBLYAI_API_KEY", "AssemblyAI speaker streaming integration requires live AssemblyAI credentials.")
	audio, format := loadSpeakerIntegrationPCM(t)
...
func speakerIntegrationOptions() stt.TranscribeOpts {
	language := speakerIntegrationLanguage()
	return stt.TranscribeOpts{
		Language: language,
...
func assertTwoSpeakerDiarization(t *testing.T, result *stt.Result) {
	t.Helper()
	if result == nil {
...
func TestIntegrationGoogleSTTStreamingTranscription(t *testing.T) {
	if !google.StreamingCredentialsPresent() {
		testutil.SkipOrFailExplicitMissingConfig(t, google.EnvCredentialsJSON+" or "+google.EnvApplicationCredentials, "Google STT streaming integration requires your own service-account or ADC credentials.")
	}
	audio, format := loadSpeakerIntegrationPCM(t)
