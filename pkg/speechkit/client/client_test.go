...
func TestClientEndpointMethodsUseExpectedRoutes(t *testing.T) {
	responses := map[string]any{
		...
		"GET /v1/catalog/providers":                        map[string]any{"provider_matrix": []map[string]any{{"provider": "openai"}}, "provider_defaults": []map[string]any{{"provider": "openai", "profileId": "stt.openai.whisper-1"}}},
	...
	calls := []struct {
		...
		{"CatalogProviders", func() error { _, _, err := c.CatalogProviders(ctx); return err }},
