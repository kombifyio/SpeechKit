...
func TestSkillContext_SetGetClear(t *testing.T) {
	...
	store.Set("alice", "timer", map[string]string{"awaiting": "duration"})
	...
	if got.Intent != "timer" {
		t.Errorf("Intent = %q, want timer", got.Intent)
	}
...
func TestSkillContext_DefaultsAreSafe(t *testing.T) {
	...
	store.Set("c", "timer", map[string]string{"k": "v"})
