...
//
// The built-in catalog covers the text utilities (copy, insert, summarize,
// quick note) and the Voice-Companion intents in English and German, plus
...
// Registry is the ordered phrase and filler catalog a Resolver matches
// against. Construct with NewRegistry (empty) or DefaultRegistry (a private
// copy of the built-in catalog); the zero value is usable and empty.
...
// DefaultRegistry returns a private copy of the built-in catalog that the
// caller may extend without affecting DefaultResolver.
func DefaultRegistry() *Registry {
	return defaultRegistry.clone()
}

// RegisterLexicon adds every phrase of lexicon under its locale. Phrases
// are normalised the same way transcripts are; empty phrases and lexicons
// without an intent are ignored. Later registrations win ties only after
// priority and phrase length.
...
// RegisterLeadingFillers registers words that may precede a phrase in
// locale ("bitte", "please") and are stripped before matching. A
// transcript consisting only of a filler resolves to nothing.
...
var defaultLexicons = []IntentLexicon{
	...
	// Resolution.Payload. See matchPhrase in resolver.go.
	{
		Intent:  IntentTime,
		Locale:  "en",
