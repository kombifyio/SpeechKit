...
func (s *HomeAssistantSkill) Execute(ctx context.Context, call assist.ToolCall) (assist.ToolResult, error) {
	...
	case "action_done":
		...
		return homeAssistantTerminalResult(result.Speech, call.Locale, "execute", assist.KindUtilityAction), nil
	case "query_answer":
		if result.Speech == "" {
			return homeAssistantFailureResult(call.Locale, "empty_response"), nil
		}
		return homeAssistantTerminalResult(result.Speech, call.Locale, "respond", assist.KindAnswer), nil
	case "error":
		if result.Speech != "" {
			return homeAssistantTerminalResult(result.Speech, call.Locale, "respond", assist.KindUtilityAction), nil
		}
...
func homeAssistantTerminalResult(text, locale, action, kind string) assist.ToolResult {
...
func homeAssistantFailureResult(locale, reason string) assist.ToolResult {
	...
	result := homeAssistantTerminalResult(message.Text, message.Locale, "respond", assist.KindUtilityAction)
