...
func (s *stubFallback) ExecuteTool(_ context.Context, _ assist.ToolCall) (assist.ToolResult, error) {
	s.called = true
	return s.result, s.err
}
...
func TestCompositeExecutor_DispatchesByIntent(t *testing.T) {
	...
	got, err := exec.ExecuteTool(context.Background(), assist.ToolCall{Intent: string(shortcuts.IntentMath)})
	if err != nil {
...
func TestCompositeExecutor_FallbackOnUnknownIntent(t *testing.T) {
	...
	got, err := exec.ExecuteTool(context.Background(), assist.ToolCall{Intent: string(shortcuts.IntentCopyLast)})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !fallback.called {
...
func TestCompositeExecutor_ErrorWhenNoSkillNoFallback(t *testing.T) {
	...
	_, err := exec.ExecuteTool(context.Background(), assist.ToolCall{Intent: string(shortcuts.IntentTime)})
	if err == nil {
...
func TestCompositeExecutor_LastRegisteredWinsForSameIntent(t *testing.T) {
	...
	got, err := exec.ExecuteTool(context.Background(), assist.ToolCall{Intent: string(shortcuts.IntentTime)})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got.Text != "second" {
...
func TestDefaultSkills_CoverPhase1aIntents(t *testing.T) {
	...
	for _, intent := range want {
		_, err := exec.ExecuteTool(context.Background(), assist.ToolCall{Intent: string(intent), Payload: "42"})
		if err != nil {
