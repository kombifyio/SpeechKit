...
func TestDeepgramParseConversationText(t *testing.T) {
	...
	if msg.EventType != live.LiveEventInputFinal || msg.ProviderMetadata["provider_event"] != "ConversationText" {
	...
	if msg.EventType != live.LiveEventOutputText || msg.ProviderMetadata["provider_event"] != "ConversationText" {
...
func TestDeepgramParseControlEvents(t *testing.T) {
	...
	if started == nil || !started.Interrupted || started.EventType != live.LiveEventInterrupted {
	...
	for _, typ := range []string{"StartOfTurn", "TurnResumed"} {
		...
		if err != nil || swallow || turn == nil || !turn.Interrupted || turn.EventType != live.LiveEventInterrupted {
	...
	for _, typ := range []string{"EagerEndOfTurn", "EndOfTurn"} {
		turn, swallow, err := p.parseEvent(mustMarshal(t, map[string]any{"type": typ}))
		if err != nil || swallow || turn == nil || turn.EventType != live.LiveEventTurnEnd {
	...
	if done == nil || !done.Done || done.EventType != live.LiveEventTurnEnd {
...
func TestDeepgramParseFunctionCall(t *testing.T) {
	...
	if msg.EventType != live.LiveEventToolCall || msg.ProviderMetadata["provider_event"] != "FunctionCallRequest" {
...
func TestDeepgramBuildSettingsManagedThink(t *testing.T) {
	...
	think := deepgramThinkFromSettings(t, (&Provider{}).buildSettings(live.LiveConfig{}))
	...
	think = deepgramThinkFromSettings(t, p.buildSettings(live.LiveConfig{}))
...
func TestDeepgramBuildSettingsMapsProviderNeutralLiveOptions(t *testing.T) {
	t.Parallel()
	settings := (&Provider{}).buildSettings(live.LiveConfig{
...
func TestDeepgramBuildSettingsOmitsAgentLanguageForFluxListen(t *testing.T) {
	t.Parallel()
	settings := (&Provider{}).buildSettings(live.LiveConfig{Locale: "en-US"})
...
func TestDeepgramBuildSettingsKeepsAgentLanguageForNonFluxListen(t *testing.T) {
	t.Parallel()
	settings := (&Provider{ListenModel: "nova-3"}).buildSettings(live.LiveConfig{Locale: "de-DE"})
	agent := settings["agent"].(map[string]any)
...
func TestDeepgramResolveSpeakModel(t *testing.T) {
	...
	configured := &Provider{SpeakModel: "aura-2-zeus-en"}
...
func TestDeepgramResolveSpeakModelFluxLanguageGate(t *testing.T) {
	t.Parallel()
	p := &Provider{SpeakModel: deepgramSpeakModelFluxEN}
	cases := []struct {
		name   string
		locale string
		hints  []string
	...
	if got := (&Provider{}).resolveSpeakModel("de-DE", "flux-haley-en", nil); got != deepgramSpeakModelDefaultDE {
		t.Errorf("explicit Flux voice on a German session: got %q, want %q", got, deepgramSpeakModelDefaultDE)
	}
	if got := (&Provider{}).resolveSpeakModel("en-US", "flux-haley-en", nil); got != "flux-haley-en" {
...
func TestDeepgramBuildSpeak(t *testing.T) {
	...
	aura := (&Provider{}).buildSpeak("de-DE", "", []string{"de"})
...
func TestDeepgramBuildSettingsFluxListenTuning(t *testing.T) {
	t.Parallel()
	p := &Provider{EOTThreshold: 0.95, EagerEOTThreshold: 0.4, EOTTimeoutMs: 3000}
	listen := deepgramListenProviderFromSettings(t, p.buildSettings(live.LiveConfig{Locale: "en-US"}))
	...
	nova := deepgramListenProviderFromSettings(t, (&Provider{ListenModel: "nova-3", EOTThreshold: 0.6}).
		buildSettings(live.LiveConfig{Locale: "en-US"}))
	...
	unset := deepgramListenProviderFromSettings(t, (&Provider{}).buildSettings(live.LiveConfig{Locale: "en-US"}))
...
func TestComposeDeepgramPrompt(t *testing.T) {
	t.Parallel()
	got := composeDeepgramPrompt(live.LiveConfig{FrameworkPrompt: "be concise", RefinementPrompt: "in German"})
	if got != "be concise\n\nin German" {
		t.Errorf("merged prompt mismatch: %q", got)
	}
	if got := composeDeepgramPrompt(live.LiveConfig{RefinementPrompt: "only this"}); got != "only this" {
