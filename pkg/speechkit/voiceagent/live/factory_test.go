...
func TestNormalizeProviderIDAliases(t *testing.T) {
	...
	for in, want := range cases {
		if got := NormalizeProviderID(in); got != want {
			t.Fatalf("NormalizeProviderID(%q) = %q, want %q", in, got, want)
		}
	}
}
...
func TestDefaultLiveConfigForProvider(t *testing.T) {
	t.Parallel()
	cfg, ok := DefaultLiveConfigForProvider("realtime.assemblyai.voice-agent")
	if !ok {
...
func TestNormalizeLiveConfigFillsProviderProfileAndModel(t *testing.T) {
	t.Parallel()
	cfg, err := NormalizeLiveConfig(LiveConfig{ProfileID: "realtime.deepgram.voice-agent"})
	if err != nil {
	...
	cfg, err = NormalizeLiveConfig(LiveConfig{Model: "gemini-3.1-flash-live-preview"})
	if err != nil {
...
func TestNewProviderResolvesBuiltInsAndCustomFactories(t *testing.T) {
	...
	provider, err := NewProviderWithFactories("acme", ProviderFactoryRegistry{
		"acme": func() LiveProvider { return &factoryTestProvider{} },
	})
	if err != nil {
...
func (p *factoryTestProvider) Connect(context.Context, LiveConfig) error { return nil }

func (p *factoryTestProvider) SendAudio([]byte) error { return nil }

func (p *factoryTestProvider) SendAudioStreamEnd() error { return nil }

func (p *factoryTestProvider) Receive(context.Context) (*LiveMessage, error) {
	return &LiveMessage{Done: true}, nil
}

func (p *factoryTestProvider) SendText(string) error { return nil }

func (p *factoryTestProvider) SendToolResponse(ToolResponse) error { return nil }
...
func TestNewProviderForConfigNormalizesConfig(t *testing.T) {
	t.Parallel()
	provider, cfg, err := NewProviderForConfig(LiveConfig{Provider: "openai-realtime"})
	if err != nil {
...
func TestNewProviderUnknownProvider(t *testing.T) {
	...
	if _, err := NormalizeLiveConfig(LiveConfig{}); !errors.Is(err, ErrUnknownLiveProvider) {
