...
// generatePCM produces durationMs of silent S16LE PCM at the canonical
// 16 kHz rate. Silent (all-zero) audio is sufficient for the capture
// machinery: the tests assert structure, not content.
func generatePCM(durationMs int) []byte {
	samples := (durationMs * wakeword.SampleRate) / 1000
	out := make([]byte, samples*wakeword.BytesPerSample)
	return out
}

// generatePCMWithMarker produces PCM where every sample = value. Used to
// assert that the ring buffer is rotating correctly: the values in the
// output WAV reflect the most recent ingest.
func generatePCMWithMarker(durationMs int, value int16) []byte {
	samples := (durationMs * wakeword.SampleRate) / 1000
	out := make([]byte, samples*wakeword.BytesPerSample)
	for i := 0; i < samples; i++ {
		binary.LittleEndian.PutUint16(out[i*wakeword.BytesPerSample:], uint16(value))
	}
	return out
}
...
func TestCapture_DisabledIsNoOp(t *testing.T) {
	...
	c.Trigger(wakeword.DetectionEvent{Phrase: "Hey Quby", Keyword: "hey_quby", Probability: 0.9, At: time.Now()})
	_ = c.Close()
...
func TestCapture_RejectsMissingDir(t *testing.T) {
...
func TestCapture_RejectsZeroPreRoll(t *testing.T) {
	dir := t.TempDir()
	_, err := NewCapture(CaptureConfig{
		Enabled:    true,
		Dir:        dir,
...
func TestCapture_FlushesAfterPostRoll(t *testing.T) {
	dir := t.TempDir()
	var captured []Record
	c, err := NewCapture(CaptureConfig{
		Enabled:    true,
		...
		OnWrite:    func(rec Record) { captured = append(captured, rec) },
	...
	c.Trigger(wakeword.DetectionEvent{
		Phrase:      "Hey Quby",
		Keyword:     "hey_quby",
		Probability: 0.87,
	...
	if rec.SampleRate != wakeword.SampleRate {
	...
	// 750 ms at 16 kHz mono S16 = 12000 samples x 2 bytes = 24000 data bytes.
...
func TestCapture_PostRollZero_FlushesImmediately(t *testing.T) {
	dir := t.TempDir()
	var captured []Record
	c, err := NewCapture(CaptureConfig{
		Enabled:    true,
...
func TestCapture_RingRotatesCorrectly(t *testing.T) {
	dir := t.TempDir()
	var captured []Record
	c, err := NewCapture(CaptureConfig{
		Enabled:    true,
	...
	// Feed 500 ms of marker=1, then 100 ms of marker=42. The ring should
	// retain only the most recent 100 ms (marker=42).
	c.Ingest(generatePCMWithMarker(500, 1))
	c.Ingest(generatePCMWithMarker(100, 42))
	c.Trigger(wakeword.DetectionEvent{Phrase: "X", Keyword: "x", Probability: 1.0, At: time.Now()})
...
func TestCapture_ConcurrentIngestAndTrigger(t *testing.T) {
	dir := t.TempDir()
	c, err := NewCapture(CaptureConfig{
		Enabled:    true,
		Dir:        dir,
	...
	go func() {
		for i := 0; i < 20; i++ {
			c.Trigger(wakeword.DetectionEvent{Phrase: "Hey", Keyword: "hey", Probability: 0.5, At: time.Now()})
		}
		done <- struct{}{}
	}()
	<-done
...
func TestCapture_FilenamesAreSortable(t *testing.T) {
...
func TestEncodeWAV_HeaderShape(t *testing.T) {
	...
	if binary.LittleEndian.Uint16(wav[22:24]) != uint16(wakeword.Channels) {
		t.Error("channels mismatch")
	}
	if binary.LittleEndian.Uint32(wav[24:28]) != uint32(wakeword.SampleRate) {
