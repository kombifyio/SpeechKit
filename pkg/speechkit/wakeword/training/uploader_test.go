...
func TestNewUploader_RejectsMissingFields(t *testing.T) {
	cases := []UploaderConfig{
	...
	for i, c := range cases {
		if _, err := NewUploader(c); err == nil {
...
func TestUploader_UploadsUnsentRecord(t *testing.T) {
	...
	up, err := NewUploader(UploaderConfig{
		Dir:        dir,
		ServerURL:  srv.URL,
...
func TestUploader_SkipsAlreadyUploaded(t *testing.T) {
	...
	up, _ := NewUploader(UploaderConfig{
		Dir:        dir,
		ServerURL:  srv.URL,
		Interval:   50 * time.Millisecond,
		HTTPClient: srv.Client(),
