...
`SPEECHKIT_PUBLIC_URL=http://localhost:8080`. Voice Agent session creation
returns a `ws_url` based on this public origin, not the Docker-internal service
name. WebSocket upgrades use `ws_url` plus `ws_subprotocol`.
