...
// options and delegates router assembly to the shared allproviders.BuildRouter SSOT —
...
func buildSTTRouter(cfg *config.Config) (*stt.Router, []namedProvider, []string) {
	...
	strategy := stt.Strategy(strings.TrimSpace(cfg.Routing.Strategy))
	...
	if strategy == "" {
		strategy = stt.StrategyCloudOnly
	}

	enabled := allproviders.EnabledProviders{Secrets: config.ResolveSecret}
	var notes []string
	...
	if cfg.HuggingFace.Enabled {
		...
		if token != "" {
			enabled.HuggingFace = &allproviders.HuggingFaceOpts{Model: cfg.HuggingFace.Model, Token: token}
	...
	if cfg.Providers.OpenRouter.Enabled {
		if key, source := credential("openrouter"); key != "" {
			enabled.OpenRouter = &allproviders.OpenRouterOpts{APIKey: key, Model: cfg.Providers.OpenRouter.STTModel}
	...
	if cfg.Providers.OpenAI.Enabled {
		if key, source := credential("openai"); key != "" {
			...
			enabled.OpenAI = &allproviders.OpenAIOpts{APIKey: key, Model: model}
	...
	if cfg.Providers.Groq.Enabled {
		if key, source := credential("groq"); key != "" {
			...
			enabled.Groq = &allproviders.GroqOpts{APIKey: key, Model: model}
	...
	if cfg.Providers.Deepgram.Enabled {
		if key, source := credential("deepgram"); key != "" {
			enabled.Deepgram = &allproviders.DeepgramOpts{
				APIKey:           key,
	...
	if cfg.Providers.AssemblyAI.Enabled {
		if key, source := credential("assemblyai"); key != "" {
			...
			enabled.AssemblyAI = &allproviders.AssemblyAIOpts{
				APIKey:            key,
	...
	if cfg.Providers.Google.Enabled {
		if key, source := credential("google_stt"); key != "" {
			enabled.Google = &allproviders.GoogleOpts{
				APIKey:                    key,
	...
	if cfg.VPS.Enabled && strings.TrimSpace(cfg.VPS.URL) != "" {
		...
		enabled.VPS = &allproviders.VPSOpts{URL: cfg.VPS.URL, APIKey: key, Model: cfg.VPS.Model}
	...
	if cfg.Local.Enabled && strings.TrimSpace(cfg.Local.ModelPath) != "" {
		enabled.Local = &allproviders.LocalOpts{Port: cfg.Local.Port, ModelPath: cfg.Local.ModelPath, GPU: cfg.Local.GPU}
		notes = append(notes, "Local whisper.cpp configured (not yet started; see readiness probe)")
	}

	routerCfg := allproviders.RouterConfig{
		Strategy:             strategy,
		...
		OnProviderSelected:   auditlog.STTProviderSelected,
	}
	r, ok, _ := allproviders.BuildRouter(routerCfg, enabled)
	if !ok {
