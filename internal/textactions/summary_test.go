...
func TestResolveSummarizeContextFallsBackToLastTranscription(t *testing.T) {
	input := ResolveSummarizeContext(SummarizeContext{
		Utterance:         "Summarize this",
		LastTranscription: "last transcription",
	})

	if got, want := input.Text, "last transcription"; got != want {
		t.Fatalf("Text = %q, want %q", got, want)
	}
	if got, want := input.Source, SourceLastTranscription; got != want {
		t.Fatalf("Source = %q, want %q", got, want)
	}
...
func TestResolveSummarizeContextDoesNotUseUtteranceAsSourceText(t *testing.T) {
	...
	if got, want := input.Text, ""; got != want {
		t.Fatalf("Text = %q, want empty when no source text is available", got)
	}
...
func TestSummaryToolReturnsMissingInputErrorWhenSourceTextIsEmpty(t *testing.T) {
	tool := SummaryTool{}

	got, err := tool.Run(context.Background(), Input{
		Instruction: "summarize",
	})
	if !errors.Is(err, ErrSummarizeInputUnavailable) {
		t.Fatalf("Run() error = %v, want %v", err, ErrSummarizeInputUnavailable)
	}
	if got != "" {
		t.Fatalf("Run() = %q, want empty output when no source text is available", got)
	}
}
