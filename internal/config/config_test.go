...
func TestLoadDefaults(t *testing.T) {
	...
	if cfg.HuggingFace.Enabled {
		t.Error("HuggingFace should be disabled by default")
	}
...
func TestApplyManagedIntegrationDefaultsEnablesHFWhenTokenResolvable(t *testing.T) {
	cfg := defaults()
	...
	if !changed {
		t.Fatal("expected managed defaults to enable huggingface")
	}
	if !cfg.HuggingFace.Enabled {
...
func TestApplyManagedIntegrationDefaultsRequiresExplicitOptIn(t *testing.T) {
	cfg := defaults()
	t.Setenv("HF_TOKEN", "test-token")
	...
	if changed {
		t.Fatal("managed defaults should stay disabled without explicit opt-in")
	}
	if cfg.HuggingFace.Enabled {
		t.Fatal("huggingface should remain disabled without explicit opt-in")
	}
}

func TestApplyManagedIntegrationDefaultsUsesBuildDefaultsWhenEnvIsUnset(t *testing.T) {
	prevHF := managedHFDefaultOptIn
	prevProject := managedDopplerDefaultProject
	prevConfig := managedDopplerDefaultConfig
	managedHFDefaultOptIn = "1"
	managedDopplerDefaultProject = "kombination-personal"
	managedDopplerDefaultConfig = "prd"
	t.Cleanup(func() {
		managedHFDefaultOptIn = prevHF
		managedDopplerDefaultProject = prevProject
		managedDopplerDefaultConfig = prevConfig
	})
	...
	if !changed {
		t.Fatal("expected managed defaults to enable huggingface from build defaults")
	}
	if !cfg.HuggingFace.Enabled {
		t.Fatal("huggingface should be enabled from build defaults")
	}
	if projects := dopplerProjects(); len(projects) != 1 || projects[0] != "kombination-personal" {
		t.Fatalf("projects = %v", projects)
	}
	if configs := dopplerConfigs(); len(configs) != 1 || configs[0] != "prd" {
		t.Fatalf("configs = %v", configs)
	}
}
