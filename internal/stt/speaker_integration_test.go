...
func TestIntegrationDeepgramDiarization(t *testing.T) {
	key := strings.TrimSpace(os.Getenv(config.DeepgramAPIKeyEnv))
	if key == "" {
		t.Skip(config.DeepgramAPIKeyEnv + " not set")
	}
...
func TestIntegrationAssemblyAIDiarization(t *testing.T) {
	key := strings.TrimSpace(os.Getenv(config.AssemblyAIAPIKeyEnv))
	if key == "" {
		t.Skip(config.AssemblyAIAPIKeyEnv + " not set")
	}
	audio := loadSpeakerIntegrationAudio(t)
...
func TestIntegrationGoogleSTTDiarization(t *testing.T) {
	key := firstEnvValue(config.GoogleSTTDefaultAPIKeyEnv, config.GoogleCloudSTTAPIKeyEnv, config.GoogleLegacySTTAPIKeyEnv)
	if key == "" {
		t.Skip(config.GoogleSTTDefaultAPIKeyEnv + " not set")
	}
	audio := loadSpeakerIntegrationAudio(t)
...
func TestIntegrationGoogleSTTStreamingTranscription(t *testing.T) {
	if !googleStreamingCredentialsPresent(config.GoogleApplicationCredentialsEnv, config.GoogleSTTCredentialsJSONEnv) {
		t.Skip(config.GoogleSTTCredentialsJSONEnv + " / " + config.GoogleApplicationCredentialsEnv + " not set")
	}
	audio, format := loadSpeakerIntegrationPCM(t)
