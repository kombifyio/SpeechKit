...
var validKinds = map[models.ProviderKind]bool{
	models.ProviderKindLocalBuiltIn:   true,
	models.ProviderKindLocalProvider:  true,
	models.ProviderKindCloudProvider:  true,
	models.ProviderKindDirectProvider: true,
}
...
func RunContract(t *testing.T, c Case) {
	...
	t.Run(c.Name+"/identity", func(t *testing.T) {
		...
		if k := p.Kind(); !validKinds[k] {
			t.Errorf("Kind() = %q, want one of the known models.ProviderKind values", k)
		}
	})
	...
	t.Run(c.Name+"/capabilities", func(t *testing.T) {
		...
		for _, capability := range caps {
			...
			if capability == models.CapabilityTTS {
				hasTTS = true
			}
		}
		if !hasTTS {
			t.Errorf("a TTS provider must report %q; got %v", models.CapabilityTTS, caps)
		}
	})
