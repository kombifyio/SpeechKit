...
func TestFactory_PostgresStub(t *testing.T) {
	_, err := New(StoreConfig{Backend: "postgres", PostgresDSN: "test"})
	if err == nil {
		t.Fatal("expected error for postgres backend (not implemented)")
	}
	if !strings.Contains(err.Error(), "not yet implemented") {
		t.Fatalf("error = %q, want message about not yet implemented", err.Error())
	}
}
